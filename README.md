...
**v0.1.5 — smart branching cadence**: the proposer no longer forces 2-4 fake-different candidates at every node — at depth ≥1 it can return `skip_expansion: true` for canonical steps (no design choice → execute directly). `autopilot --continuous` chains steps back-to-back without `/loop`'s 30-min sleep until every live node is blocked on background work or the session step counter (default 10 in v0.1.8, was 20) hits its threshold, at which point you're told to restart Claude Code for a clean context. Best of both: `/loop 30m /research-tree autopilot --continuous --silent` chews through quick chained work between `/loop` ticks but never accumulates more than ~10 steps of context per session.

**v0.1.8 — human-gate fast-exit**: when autopilot needs a human decision (session-cap, DONE, ROOT_FAILURE, etc.) it raises a sentinel file `.research-tree/AWAITING_HUMAN.md`. On every subsequent `/loop` tick autopilot's new Step 0 short-circuits before running any orchestration — in `--silent` mode this means zero main-context tokens are spent while waiting on the human. Previously each tick re-printed a multi-line "Session approaching capacity" warning, which compounded into context-window pressure during long unattended runs. Resume with `/research-tree resume` (the only path that clears the gate).
