...
def cmd_set(args) -> None:
    ...
    if "status" in updates and updates["status"] not in VALID_STATUSES:
        sys.exit(f"ERROR: invalid status {updates['status']!r}. Valid: {sorted(VALID_STATUSES)}")

    prev_status = node["status"]
    node.update(updates)
    node["updated_at"] = now_iso()

    # update stats if status transition
    if "status" in updates and updates["status"] != prev_status:
        was_alive = prev_status in ("pending", "expanded", "running")
        now_alive = updates["status"] in ("pending", "expanded", "running")
        if was_alive and not now_alive:
            state["stats"]["nodes_alive"] -= 1
        elif not was_alive and now_alive:
            state["stats"]["nodes_alive"] += 1
        if updates["status"] == "dead" and prev_status != "dead":
            state["stats"]["nodes_dead"] += 1
        if updates["status"] == "completed" and prev_status != "completed":
...
def main() -> None:
    ...
    p_set = sub.add_parser("set", help="update node fields")
